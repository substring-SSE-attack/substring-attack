...
def parse_files(folder):
    ...
    for file_name in os.listdir(folder):
        ...
        if N_doc not in results:
            results[N_doc] = {'token_unique': [0,0],
                                'token_rep':    [0,0],
                                'token_seq':    [0,0],
                                'query_full':   [0,0],}
        ...
        results[N_doc]['token_unique'][0] += int(line[0])
        results[N_doc]['token_unique'][1] += int(line[1])

        line = file_input.readline().split(',')
        results[N_doc]['token_seq'][0] += int(line[0])
        results[N_doc]['token_seq'][1] += int(line[1])

        line = file_input.readline().split(',')
        results[N_doc]['query_full'][0] += int(line[0])
        results[N_doc]['query_full'][1] += int(line[1])

        line = file_input.readline().split(',')
        results[N_doc]['token_rep'][0] += int(line[0])
        results[N_doc]['token_rep'][1] += int(line[1])
